...
if (!function_exists('system_error_log')) {
    ...
    if (!function_exists('system_error_short_description')) {
        function system_error_short_description(array $row): string
        {
            $source = (string)($row['source'] ?? '');
            $code = trim((string)($row['response_code'] ?? ''));
            $bankDescriptions = [
                '-1' => 'В запросе банка не заполнено обязательное поле.',
                '-2' => 'Запрос не прошёл проверку шлюза BCC.',
                '-4' => 'Сервер не смог соединиться с хостом банка.',
                '-5' => 'Соединение с банком прервалось во время операции.',
                '-10' => 'Банк отклонил запрос из-за некорректной суммы.',
                '-11' => 'Банк отклонил запрос из-за некорректной валюты.',
                '-12' => 'Идентификатор продавца не принят банком.',
                '-15' => 'В запросе указан некорректный RRN.',
                '-19' => 'Аутентификация операции не выполнена.',
                '-20' => 'Время запроса отличается от времени шлюза больше допустимого.',
                '-21' => 'Операция уже была выполнена ранее.',
                '-23' => 'Ошибка в контексте банковской транзакции.',
                '-24' => 'Данные операции не совпали с исходной транзакцией.',
                '-30' => 'Банк отклонил операцию как подозрительную.',
                '-31' => 'Операция ещё находится в обработке.',
                '-39' => 'Транзакция ожидает подтверждения.',
                '-44' => '3-D Secure недоступен.',
                '-46' => 'Некритичная ошибка банка, операцию можно повторить после проверки.',
                '03' => 'Банк не признал продавца действительным.',
                '05' => 'Банк запретил проведение операции.',
                '12' => 'Банк считает транзакцию недействительной.',
                '51' => 'Недостаточно средств или банк не подтвердил возврат.',
                '57' => 'Операция запрещена держателю карты.',
                '58' => 'Операция запрещена для терминала.',
                '61' => 'Превышен лимит суммы операции.',
                '78' => 'Исходная запись операции не найдена.',
                '82' => 'Банк клиента временно недоступен.',
                '95' => 'Ошибка согласования банковской операции.',
                '96' => 'В банке произошла системная неисправность.',
                'TRANSPORT_ERROR' => 'Сервер не получил ответ от банка по сети.',
            ];
            if ($code !== '' && isset($bankDescriptions[$code])) {
                return $bankDescriptions[$code];
            }
            if ($source === 'php.error') {
                return 'PHP обнаружил ошибку во время выполнения запроса.';
            }
            if ($source === 'php.exception') {
                return 'Произошло необработанное исключение, выполнение запроса было прервано.';
            }
            if ($source === 'php.shutdown') {
                return 'PHP завершил выполнение из-за критической ошибки.';
            }
            if (str_starts_with($source, 'bcc.')) {
                return 'Ошибка при обмене данными с банком или обработке банковского ответа.';
            }
            return 'Ошибка приложения. Откройте подробности и проверьте технический контекст.';
        }
    }
}
