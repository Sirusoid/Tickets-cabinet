...
if (!function_exists('order_email_build_order_message')) {
    function order_email_build_order_message(PDO $pdo, string $email, string $order, bool $respectEnabled = true): array
    {
        ...
        $html = '<!doctype html><html lang="ru"><head><meta name="color-scheme" content="light only"><meta name="supported-color-schemes" content="light"></head><body bgcolor="#ffffff" style="margin:0;background:#ffffff!important;font-family:Arial,sans-serif;color:#172b4d;line-height:1.5">'
            ...
            . '<td valign="middle" style="padding:0 90px 0 0;width:100px">' . $logoHtml . '</td>'
            ...
            . '<a href="' . $safeOrderUrl . '" target="_blank" style="display:inline-block;padding:13px 20px;border-radius:9px;background:#18a957;color:#ffffff!important;text-decoration:none;font-weight:700">Открыть страницу заказа</a>'
            . '</td></tr></table>'
            . '<p style="margin:10px 0 0;color:#64748b;font-size:12px">Если кнопка не открывается, воспользуйтесь ссылкой:</p>'
            . '<p style="margin:4px 0 0;word-break:break-all;font-size:12px"><a href="' . $safeOrderUrl . '" target="_blank" style="color:#1d4ed8">' . $safeOrderUrl . '</a></p>'
